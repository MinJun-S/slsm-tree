...
public:
    ...
    void save_run();
    void save_file();
    set<int> Create_Query_filter(VAL_t, float);           // create query filter
	unsigned int zorder2D(unsigned, unsigned);           // compute z-order curve for create query filter
	unsigned int zorder2D_X(unsigned);
	unsigned int zorder2D_Y(unsigned);
    void reset_Q_filter();
