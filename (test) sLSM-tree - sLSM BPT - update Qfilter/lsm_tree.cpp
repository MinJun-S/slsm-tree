...
/* For Create_Query_filter */
static const unsigned int B[] = { 0x55555555, 0x33333333, 0x0F0F0F0F, 0x00FF00FF };
static const unsigned int S[] = { 1, 2, 4, 8 };
unsigned int LSMTree::zorder2D(unsigned x, unsigned y) {
...
unsigned int LSMTree::zorder2D_X(unsigned x) {
...
unsigned int LSMTree::zorder2D_Y(unsigned y) {
...
set<int> LSMTree::Create_Query_filter(VAL_t query_val, float range)
{
	// compute z order curve original code link :  https://stackoverflow.com/questions/12157685/z-order-curve-coordinates
	...
	for (unsigned y = 0; y < ny; y++) {
		for (unsigned x = 0; x < nx; x++) {
			...
			min_key.x = (x_size * x) + GEO_X_MIN;
			min_key.y = (y_size * y) + GEO_Y_MIN;
			max_key.x = ((x_size * (x + 1))) + GEO_X_MIN - 0.001;
			max_key.y = ((y_size * (y + 1))) + GEO_Y_MIN - 0.001;

			measure = Compute_Overlap(lb, ub, min_key, max_key);

			if (measure > 0) {
				Q_filter.insert(res[y][x]);
			}			
			//printf("y x=%d %d     z=%d\n", y, x, res[y][x]);
		}
	}
