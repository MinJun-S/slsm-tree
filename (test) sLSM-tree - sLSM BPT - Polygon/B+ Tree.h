...
public:
	...
	vector<KEY_t> keys;
	vector<VAL_t> val;
	vector<MBR_LB_t> MBR_LB;
	vector<MBR_UB_t> MBR_UB;
	vector<POLY_t> poly_name;
...
private:
	...
	void insertInternal(KEY_t x, Node** cursor, Node** child, VAL_t val, MBR_LB_t MBR_LB, MBR_UB_t MBR_UB, POLY_t poly_name);  //Insert x from child in cursor(parent)
...
public:
	...
	int bpt_knn(KEY_t query, int distance, int bpt_IO_Check, int k);
	void bpt_insert(KEY_t key, FILE* filePtr, entry_t input_entry);
