 ...
 */
static int _RTreeInsertRect( RTREEMBR *rc, int tid,  RTREENODE *node, RTREENODE **new_node, int level)    // 2. (노드하나 가져와(처음엔 루트노드))시작할 때 이 함수 안에서 데이터를 계속 넣고, 넣다가 꽉차면 split을 시킴
{
    int i;
    RTREEBRANCH b;   // temp branch
    RTREENODE *n2;   // temp node
    ...
    if (node->level > level)
    {
        i = RTreePickBranch(rc, node);                                           // <-- 3. (골라) 여기서 현 레벨 노드의 자식 노드 중 어디 branch에 넣을지 선택!! 후에 i번째 branch에 데이터 Insert<중요> (이 함수에선 IO체크 안하는거!!)
        
		if (!_RTreeInsertRect(rc, tid, node->branch[i].child, &n2, level))       // 4. 만약 스플릿되지 않아 => MBR 늘려(=합쳐=combine) -> I/O아니야
        {			
            /* child was not split */														
        ...
        /* child was split */														// 5. if문 안들어가 = 만약 스플릿 되어야해   -> 여기도 I/O아니야
        node->branch[i].mbr = RTreeNodeCover(node->branch[i].child);                // mbr 설정해줘
...
int RTreeAddBranch( RTREEBRANCH *br, RTREENODE *node, RTREENODE **new_node, int level)
{
	...
	/* 8. 여유공간 없어 = child new_node 하나 할당해서 split해줘 (끝) */
    assert(new_node);
...
{
    RTREENODE * root = RTreeNewNode();   // 노드 생성 (여기선 root노드) -> malloc으로 잡고, branch(=child)까지 만들어둠)
    root->level = 0;					 /* 0 is leaf (= root node에서 처음 만들 경우 root노드가 leaf노드임) */
    return root;
}
...
int RTreeInsertRect( RTREEMBR *rc, int tid, RTREENODE **root, int level)
{
	...
	// 여기에 if 특정 level이면 I/O ++ ??

    RTREENODE    *newroot;
    RTREENODE    *newnode;
    RTREEBRANCH b;
    
    assert(rc && root);										// rc = rectangle (=mbr)
    ...
    /* root split (스플릿 된다면 if문 수행) */
    if (_RTreeInsertRect(rc, tid, *root, &newnode, level))    // 2. <이 함수를 봐야함>시작할 때 이 함수 안에서 데이터를 계속 넣고, 넣다가 꽉차면 split을 시킴
    {			
...
int rangeQuery(RTREENODE *node, RTREEMBR *rc, RTREEMBR *q, double radius, int *visited)
{
    int hitCount = 0;
    int i;
...
void q_push(CANDIDATE** head, CANDIDATE* candidate, int mode)
{
    ...
    else
    {
        ...
        while (tmp)
        {
            if ((!mode && tmp->distance < candidate->distance)
            || (mode && tmp->distance > candidate->distance))
            {
                candidate->next = tmp;
                if (prev)
                {
...
void kNNQuery(RTREENODE *node, RTREEMBR *q, int *hitCount, int k, CANDIDATE **head, int *visited)
{
    int i;
    RTREEMBR rc;
    ...
    if (node->level > 0)
    {
        for (i = 0; i < NODECARD; i++)
        {
            if (node->branch[i].child)
            {
                if (*hitCount < k)
                {
                    kNNQuery(node->branch[i].child, q, hitCount, k, head, visited);
                }
                else
                {
                    rc = (*head)->mbr;
