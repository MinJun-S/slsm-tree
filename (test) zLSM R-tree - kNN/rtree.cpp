...
{
	return sqrt(pow(rc1->bound[0] - rc2->bound[0], 2) + pow(rc1->bound[1] - rc2->bound[1], 2));
}
...
void q_push(CANDIDATE** head, CANDIDATE* candidate, int mode)
{
	...
	{
		CANDIDATE *tmp = *head, *prev = NULL;
		while (tmp)
		{
			if ((!mode && tmp->distance < candidate->distance) || (mode && tmp->distance > candidate->distance))
			{
				candidate->next = tmp;
				if (prev)
				{
...
CANDIDATE * q_pop(CANDIDATE **head)
{
	//printf("q_pop()!\n");
	if (!(*head))
	{
