...
void command_loop(LSMTree& tree) {
    ...
    if (file == NULL) {
        ...
        cout << "\n* Success z-LSM R-Tree building with " << i << " point data!" << endl;
        fclose(file);
    }
    ...
    while (cin >> command) {
        ...
        case 'i':
			tree.IO_Check = 0;
			cout << " * init I/O Check = " << tree.IO_Check << endl;
            cout << "\n* Loading File and Start z-LSM R-Tree Building... " << endl;            
            FILE * file; 
            file = fopen("src/sample_data.txt", "r");
