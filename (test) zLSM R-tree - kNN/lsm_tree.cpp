...
void LSMTree::merge_down(vector<Level>::iterator current, int idx) {
    ...
    for (const auto& entry : current->runs_list[idx]->entries) 
    {
        ...
        */
		...
		RTREEMBR mbr = { {entry.val.x, entry.val.y, 0, entry.val.x, entry.val.y, 0} };

        RTreeInsertRect(&mbr, ++num, &next->runs_list[i]->root, 0);
...
void LSMTree::put(KEY_t key, VAL_t val) {
    ...
    for (const auto& entry : buffer.entries) {
        ...
        RTreeInsertRect(&mbr, ++num, &levels.front().runs_list[i]->root, 0);
...
void LSMTree::KNN_query1(entry_t query_point, int k)
{
    ...
    for (int i = 0; i < DEFAULT_TREE_DEPTH; i++) {
        if (levels[i].runs_list[0]->entries.size() > 0) {
...
void LSMTree::BPT_KNN_query22(entry_t query_point, int k)
{
	...
	RTREEPQ tmp_resultQ = { (RTREEPQNODE*)malloc(sizeof(RTREEPQNODE) * 10000), 0, 10000 , -1 };
	...
	for (int i = 0; i < DEFAULT_TREE_DEPTH; i++)
	{
		...
		kNNQuery_Rtree(root, &q, &hitCount, k, &resultQ, &visited);
