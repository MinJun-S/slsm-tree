...
#pragma warning( disable : 4311 )
    unsigned int node_int = reinterpret_cast<std::uintptr_t>(node);
    fprintf(stdout, "  level=%d  count=%d  address=%o ", node->level, node->count, node_int);
#pragma warning(pop)
...
#pragma warning( disable : 4311 )
        for (i = 0; i < LEAFCARD; i++)
        {
            ...
            {
                hitCount++;
                int node_int = reinterpret_cast<std::uintptr_t>(node->branch[i].child);
                printf("Find mbr %d: %lf, %lf\n", node_int, node->branch[i].mbr.bound[0], node->branch[i].mbr.bound[1]);
                /* call the user-provided callback and return if callback wants to terminate search early */
                if (pfnSHCB && !pfnSHCB(node_int, pfnParam))
                    return hitCount;
...
#ifdef _DEBUG
    //for (i = 0; i < DIMS_NUMB; i++)
        //printf("%lf, %lf\n", rc->bound[i], rc->bound[DIMS_NUMB + i]);
...
#pragma warning( disable : 4311 )
            for (i = 0; i < MAXKIDS(tmp_nptr); i++)
            {
                ...
                {
                    int node_int = reinterpret_cast<std::uintptr_t>(tmp_nptr->branch[i].child);
                    RTreeInsertRect(&(tmp_nptr->branch[i].mbr), node_int, root, tmp_nptr->level);
                }
            }
#pragma warning(pop)
...
REALTYPE Mindist(RTREEMBR* R, RTREEMBR* p)
{

    double r, dist = 0;
    for (int i = 0; i < 2; i++) {
        if (p->bound[i] < R->bound[i]) {
            r = R->bound[i];
        }
        else if (p->bound[i] > R->bound[i + DIMS_NUMB]) {
            r = R->bound[i + DIMS_NUMB];
        }
        else {
            r = p->bound[i];
        }
        dist += pow(p->bound[i] - r, 2);
    }
    return sqrt(dist);
}

REALTYPE MinMaxDist(RTREEMBR* R, RTREEMBR* p)
{
    double rmk, rMi;
    double dist[2];
    for (int i = 0; i < 2; i++) {
        rmk = p->bound[i] < (R->bound[i] + R->bound[i + DIMS_NUMB]) / 2.0 ? R->bound[i] : R->bound[i + DIMS_NUMB];
        rMi = p->bound[1 - i] < (R->bound[1 - i] + R->bound[1 - i + DIMS_NUMB]) / 2.0 ? R->bound[1 - i] : R->bound[1 - i + DIMS_NUMB];
        dist[i] = pow(p->bound[i] - rmk, 2) + pow(p->bound[1 - i] - rMi, 2);
    }
    return MIN(dist[0], dist[1]);
}

void RTreeStackPush(RTREESTACK* stack, RTREENODE* node)
{
    if (stack->size == 0) {
        //stack->nodes = (RTREENODE**)malloc(sizeof(RTREENODE*));
    }
    else {
        stack->nodes = (RTREENODE**)realloc(stack->nodes, sizeof(RTREENODE*) * (stack->size + 1));
    }
    stack->nodes[stack->size] = node;
    stack->size++;
}

RTREENODE* RTreeStackPop(RTREESTACK* stack)
{
    assert(stack->size > 0);
    RTREENODE* ret = stack->nodes[stack->size - 1];
    stack->size--;
    if (stack->size != 0) {
        stack->nodes = (RTREENODE**)realloc(stack->nodes, sizeof(RTREENODE*) * stack->size);
    }
    return ret;
}

void RTreePQPush(RTREEPQ* pq, RTREEPQNODE node)
{
    int size = pq->cur_size;
    if (size >= pq->max_size_rtree) return;
    ...
    while (1) {
        ...
        if (pq->heap[current].mindist * pq->order > pq->heap[parent].mindist * pq->order) {
            RTREEPQNODE temp = pq->heap[current];
...
RTREEPQNODE RTreePQPop(RTREEPQ* pq)
{
    ...
    while (left < size) {
        if (left < pq->cur_size && pq->heap[maxNode].mindist * order < pq->heap[left].mindist * order) {
            maxNode = left;
        }
        if (right < pq->cur_size && pq->heap[maxNode].mindist * order < pq->heap[right].mindist * order) {
            maxNode = right;
        }
...
void RTreeKNNQuery(RTREENODE** root, RTREEMBR q, int K)
{
    assert(K > 0);
    RTREEPQ resultQ = { (RTREEPQNODE*)malloc(sizeof(RTREEPQNODE) * K), 0, K , 1 };
    RTREEPQ nodeQ = { (RTREEPQNODE*)malloc(sizeof(RTREEPQNODE) * 10000), 0, 10000 , -1 };
    double cur_radius = 1000000.0;
    REALTYPE tmp_mindist, tmp_minmax_dist;


    RTREEPQNODE root_node = { *root, Mindist(&(*root)->branch->mbr, &q), MinMaxDist(&(*root)->branch->mbr, &q) };
    RTreePQPush(&nodeQ, root_node);

    RTREEPQNODE max_nn;

    while (nodeQ.cur_size != 0) {
        RTREEPQNODE cur_node = RTreePQPop(&nodeQ);
        for (int i = 0; i < cur_node.obj.node->count; i++) {
            tmp_mindist = Mindist(&(cur_node.obj.node->branch[i].mbr), &q);
            tmp_minmax_dist = MinMaxDist(&(cur_node.obj.node->branch[i].mbr), &q);


            // if current node is leaf node
            if (cur_node.obj.node->level == 0) {
                RTREEMBR* mbr = &(cur_node.obj.node->branch[i].mbr);
                RTREEPQNODE new_node = { (RTREENODE*)mbr, tmp_mindist, tmp_minmax_dist };
                if (resultQ.cur_size == K) {
                    max_nn = RTreePQPop(&resultQ);
                    cur_radius = max_nn.mindist;
                    //fprintf(stdout, "RTree KNNQuery radius : % lf \n", cur_radius);
                }

                if (tmp_mindist <= cur_radius) {
                    RTreePQPush(&resultQ, new_node);
                }
                else {
                    RTreePQPush(&resultQ, max_nn);
                }
            }
            else if (cur_node.obj.node->level > 0 && tmp_mindist <= cur_radius) {
                RTREEPQNODE new_node = { (cur_node.obj.node->branch[i].child), tmp_mindist, tmp_minmax_dist };
                if (tmp_mindist < cur_radius) {
                    RTreePQPush(&nodeQ, new_node);
                }
            }
        }
    }

    //printf("RTree KNNQuery time: %lf", end - start);

    //for (int i = 0; i < K; i++) {
    //    RTREEPQNODE cur_result = RTreePQPop(&resultQ);
    //    printf("mbr %lf, %lf in dist %lf\n", cur_result.obj.mbr->bound[0], cur_result.obj.mbr->bound[1], cur_result.mindist);
    //}
}